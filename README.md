...
`tools/list` at `https://gateway.pipeworx.io/gdc/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/gdc_list_projects \
  -H 'Content-Type: application/json' \
  -d '{"size":20}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/gdc_list_projects`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
